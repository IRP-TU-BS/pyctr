...
class VisPy(Engine):
    def __init__(self):
        self.canvas = scene.SceneCanvas(title='PyCTCR - Vispy', size=(600, 600),
    ...
    def displ_fwd(self, frame, name="robot"):
        ...
        while cur_tube_num <= number_of_tubes and len(tmp_segs) != 0:
            ...
            if seg[0] == cur_tube_num:
                ...
                tube = scene.visuals.Tube(points= pos[:seg[1],:], radius=self.radii[cur_tube_num-1] * 1e3, parent=self.canvas.scene)
                if len(self.tubes[name]) > 0:
    ...
    def displ_fwd(self,frame, name="robot", show_forces=False):
        ...
        while cur_tube_num <= number_of_tubes and len(tmp_segs) != 0:
            ...
            if seg[0] == cur_tube_num:
                ...
                self.plotted_robots[name][name + 'tube' + str(cur_tube_num) + '_shade'].set_data(
                    pos[:seg[1], 0],
                    pos[:seg[1], 1],
                    )
        ...
        if show_forces:
            for i, p in enumerate(pos):
                transf = np.vstack([np.hstack([rots[i].reshape(3,3),p.reshape(-1,1)]),np.array([0,0,0,1])])
                f_vector = np.hstack([wrenches[i][:3], np.ones(1)]).reshape(-1,1)
                f_vector = transf@f_vector
                self.add_arrow(f_vector[0,0], f_vector[1,0], f_vector[2,0], p[0],p[1],p[2], name=i)
    ...
    def show(self, frames, show_forces):
        ...
        for name in frames.keys():
            self.ani[name] = FuncAnimation(
                self.fig, partial(self.displ_fwd, name=name, show_forces=show_forces),
                frames=frames[name],
                blit=False)
        plt.show()
...
class Scene:
    ...
    def update(self):
        for rob in self.robots.keys():
            ...
            self.robot_frames[rob].append([positions, segments[0], number_of_tubes, rotations, wrenches])
