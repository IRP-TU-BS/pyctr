...
class CosseratRod:
    ...
    def __init__(self, params=None):
        self.params = {
            ...
            "r_inner": 0.0,  # Cross-sectional radius
            "r_outer": 1.4 * 1e-3,  # m
        ...
        self.params["A"] = np.pi * (
            self.params["r_outer"] ** 2 - self.params["r_inner"] ** 2
        )  # Cross-sectional area
        self.params["I"] = (
            np.pi / 4 * (self.params["r_outer"] ** 4 - self.params["r_inner"] ** 4)
        )  # Area moment of inertia
        self.params["J"] = (
            np.pi
            * ((2 * self.params["r_outer"]) ** 4 - (2 * self.params["r_inner"]) ** 4)
        ) / 32.0  # Polar moment of inertia

        self.params["Kse"] = np.diag(
            [
                self.params["G"] * self.params["A"],
                self.params["G"] * self.params["A"],
                self.params["E"] * self.params["A"],
            ]
        )  # Stiffness matrices
        self.params["Kbt"] = np.diag(
            [
                self.params["E"] * self.params["I"],
                self.params["E"] * self.params["I"],
    ...
    def set_initial_conditions(self, p0, R0):
        self.inital_conditions = {"p0": p0, "R0": R0}
    ...
    def cosserat_rod_ode_curvature(self, state, s):
        ...
        Rs = R.dot(hat(u))
        ns = -self.params["rho"] * self.params["A"] * self.params["g"].T
        us = -np.linalg.inv(self.params["Kbt"]) @ (
            hat(u) @ (self.params["Kbt"] @ u) + hat(self._e3.T) @ R.T @ n
        )
    ...
    def external_gauss_forces(self, s):
        return np.array(
            [[0, 0, 0]]
        ).T  # TODO workaround - should be called external forces and should be modular
    ...
    def shooting_function(self, guess):
        ...
        y0 = np.hstack(
            [
                self.inital_conditions["p0"][0],
                np.reshape(self.inital_conditions["R0"], (1, 9))[0],
                n0,
                m0,
                u,
            ]
    ...
    def push_end(self, wrench, curvature_integration=False):
        ...
        solution_bvp = least_squares(
            self.shooting_function_force,
            state[0],
            method="lm",
            loss="linear",
            ftol=1e-6,
            args=(curvature_integration,),
        )
        steps = int(np.ceil(self.params["L"] / self.params["s"]))
    ...
    def move_end(self, p, R):
        ...
        _, _ = self.inital_conditions["p0"], self.inital_conditions["R0"]
        state = np.zeros((1, 6))

        solution_bvp = least_squares(
            self.shooting_function, state[0], method="lm", loss="linear"
        )
    ...
    def calc_m_from_curvature(self, R, u):
        ...
        for i in range(R.shape[0]):
            m[i, :] = (
                R[i].reshape(3, 3) @ self.params["Kbt"] @ u[i].reshape(3, -1)
            ).flatten()  # hookes law
        return m
    ...
    def is_curved_or_at_end(self, s):
        if (
            s
            <= self.params["straight_length"]
            - self.params["straight_length"] * self.params["beta"]
        ):
            return 0
...
class CurvedCosseratRod(CosseratRod):
    ...
    def cosserate_rod_ode(self, state, s):
        ...
        m = state[15:]
        u = (
            np.dot(np.linalg.inv(self.params["Kbt"]).dot(R.T), m) + self.get_kappa()
        )  # TODO research
    ...
    def calc_m_from_curvature(self, R, u):
        ...
        for i in range(R.shape[0]):
            m[i, :] = (
                R[i].reshape(3, 3)
                @ self.params["Kbt"]
                @ (u[i].reshape(3, -1) - self.get_kappa().reshape(3, -1))
            ).flatten()  # hookes law
        return m
    ...
    def apply_force(self, wrench, steps=100, curvature_integration=False):
        ...
        step_size_straight = int(
            steps * self.params["straight_length"] / self.params["L"]
        )  # get how many steps we need to integrate the singel tube
        step_size_curved = int(
            steps
            * (self.params["L"] - self.params["straight_length"])
            / self.params["L"]
        )  # get curved steps as we use ODE
        # straight part
        p0, R0 = (
            self.inital_conditions["p0"],
            self.inital_conditions["R0"],
        )  # get start from initial condition
        ...
        if step_size_straight > 0:
            self.set_kappa(
                0
            )  # set kappa to 0 as we are in the straight part of the tube
            state = np.hstack(
                [p0.flatten(), R0.reshape((1, 9)).flatten(), wrench]
            )  # initial state
            s = np.linspace(0, self.params["straight_length"], step_size_straight)
            straight_states = integrate.odeint(
                self.cosserate_rod_ode, state, s
            )  # integrate over s steps
        ...
        state = np.hstack(
            [p0.flatten(), R0.reshape((1, 9)).flatten(), wrench]
        )  # take state from straight parts last step
